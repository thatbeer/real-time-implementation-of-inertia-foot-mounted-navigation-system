...
class INS:
    ...
    def baseline(self, u, zupt, logL, adpt_flag):
        N = len(u[0])
        P, Q, R, H = self.init_filter()  # Initialization of filter
    ...
    def init_filter(self): # checked
        if self.simdata['scalefactors'] == 'on' and self.simdata['biases'] == 'on':
            ...
            P[9:12, 9:12] = np.diag(self.simdata['sigma_initial_acc_bias'].ravel() ** 2)
            P[12:15, 12:15] = np.diag(self.simdata['sigma_initial_gyro_bias'].ravel() ** 2)
            P[15:18, 15:18] = np.diag(self.simdata['sigma_initial_acc_scale'].ravel() ** 2)
            P[18:21, 18:21] = np.diag(self.simdata['sigma_initial_gyro_scale'].ravel() ** 2)
            Q = np.zeros((12, 12))
            Q[6:9, 6:9] = np.diag(self.simdata['acc_bias_driving_noise'].ravel() ** 2)
            Q[9:12, 9:12] = np.diag(self.simdata['gyro_bias_driving_noise'].ravel() ** 2)
            H = np.zeros((3, 9 + 6 + 6))
        elif self.simdata['scalefactors'] == 'on' and self.simdata['biases'] == 'off':
            P = np.zeros((9 + 6, 9 + 6))
            P[9:12, 9:12] = np.diag(self.simdata['sigma_initial_acc_scale'].ravel() ** 2)
            P[12:15, 12:15] = np.diag(self.simdata['sigma_initial_gyro_scale'].ravel() ** 2)
            Q = np.zeros((6, 6))
            H = np.zeros((3, 9 + 6))
        elif self.simdata['scalefactors'] == 'off' and self.simdata['biases'] == 'on':
            P = np.zeros((9 + 6, 9 + 6))
